SELECT * FROM TBL_BUS;
SELECT * FROM TBL_TRAIN where startsta ='여수EXPO';
SELECT * FROM TBL_CAMP;
SELECT * FROM TBL_AIR;
SELECT * FROM TBL_PARK;
SELECT * FROM TBL_like;
SELECT * FROM tbl_reserve order by no desc;
SELECT * FROM tbl_weather where dong like '%면';

deletE tbl_board;

SELECT NO, TITLE, CONT, READCOUNT, CREATEDATE, MODIFYDATE, MEMBERNO FROM
(SELECT ROWNUM NUM, NO, TITLE, CONT, READCOUNT, CREATEDATE, MODIFYDATE, MEMBERNO FROM 
(SELECT * FROM TBL_BOARD WHERE TITLE LIKE '%%' OR CONT LIKE '%%' ORDER BY NO DESC))
...
BEGIN
    FOR N IN 1..115
    LOOP
        INSERT INTO TBL_BOARD VALUES(BOARD_SEQ.NEXTVAL, '게시글 '||BOARD_SEQ.CURRVAL , '게시글 테스트입니다.'||BOARD_SEQ.CURRVAL, BOARD_SEQ.CURRVAL, SYSDATE, SYSDATE,'test',2);
    END LOOP;
...
SELECT * FROM TBL_TRAIN WHERE startsta='공주' and ENDSTA = '여수EXPO';
SELECT startsta FROM TBL_TRAIN where endsta = '서울' GROUP BY startsta;
SELECT ENDSTA,endlat,endlng FROM TBL_train GROUP BY ENDSTA,endlat,endlng;
SELECT ENDSTA FROM TBL_AIR GROUP BY ENDSTA;
...
ALTER TABLE TBL_CAMP MODIFY INTRO VARCHAR2(4000);
...
INSERT INTO TBL_PHOTO VALUES(PHOTO_SEQ.NEXTVAL,'제목','내용',100,'공원','','',SYSDATE,SYSDATE,2);
...
INSERT INTO TBL_CAMP VALUES(0,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
...
CREATE TABLE "TBL_RESERVE" (
    ...
    STARTTIME VARCHAR2(1000) NULL,
    TID VARCHAR2(1000) NULL,
	COUNT	VARCHAR2(1000)		NULL,
	PRICE	NUMBER		NULL,
...
ALTER TABLE "TBL_REVIEW" ADD CONSTRAINT "FK_TBL_MEMBER_TO_TBL_REVIEW" FOREIGN KEY (
	MEMBERNO
)
REFERENCES "TBL_MEMBER" (
	NO
...
ALTER TABLE "TBL_REVIEW" ADD CONSTRAINT "FK_TBL_FESTIVAL_TO_TBL_REVIEW" FOREIGN KEY (
	FESTIVALNO
)
REFERENCES "TBL_FESTIVAL" (
	NO
